...
def extract_delta_Q_min_mean(batch,index,start_cycle,end_cycle):
    ...
    for ind in index:
        cell_no = list(batch.keys())[ind]
        Qd_100 = batch[cell_no]['cycles'][str(end_cycle-1)]['Qclin']
        Qd_10 = batch[cell_no]['cycles'][str(start_cycle-1)]['Qclin']
        delta = Qd_100-Qd_10
...
def extract_cycle_QDischarge(batch,index,cycle):
    ...
    for ind in index:
        cell_no = list(batch.keys())[ind]
        qd_ = batch[cell_no]['summary']['QC'][cycle-1]
        qd_ = log(abs(qd_),10)
        X.append(qd_)
    X = np.reshape(X,(-1,1))
    return X
...
def extract_max_minus_cycle_2_QDischarge(batch,index):
    ...
    for ind in index:
        cell_no = list(batch.keys())[ind]
        max_minus_2 = max(batch[cell_no]['summary']['QC'][0:100]) - batch[cell_no]['summary']['QC'][1]
...
def extract_slope_intercept_cycle_to_cycle(batch,index,start_cycle,end_cycle):
    ...
    for ind in index:
        ...
        y_axis = batch[cell_no]['summary']['QC'][start_cycle-1:end_cycle]
...
def linear_interpolate_array(input_array, target_length):
    # 确保输入数组至少有两个数据点
    if len(input_array) < 2:
        raise ValueError("输入数组至少需要包含两个数据点")

    # 计算输入数组中每个数据点之间的间隔
    interval = (len(input_array) - 1)
    ...
    for i in range(target_length):
        # 计算插值点在输入数组中的索引位置
        index = i * interval / (target_length - 1)
        lower_index = int(np.floor(index))
        upper_index = int(np.ceil(index))

        if upper_index >= len(input_array):
            interpolated_value = input_array[-1]
        else:
            weight = index - lower_index
            interpolated_value = input_array[lower_index] + weight * (input_array[upper_index] - input_array[lower_index])

        output_array.append(interpolated_value)

    return output_array


def add_bat_Qclin(dic_dict):
    for name in dic_dict.keys():
        for idx in dic_dict[name]['cycles'].keys():
            value=linear_interpolate_array(dic_dict[name]['cycles'][idx]['Qc'],1000)
            dic_dict[name]['cycles'][idx]['Qclin']=np.array(value)
    return dic_dict
def main():
    # batch3 = pickle.load(open('../Data/batch3_corrected.pkl', 'rb'))
    # with open('../Data/batch3_corrected_Qclin.pkl', 'wb') as fp:
    #     pickle.dump(add_bat_Qclin(batch3), fp)
    #
    # batch2 = pickle.load(open('../Data/batch2_corrected.pkl', 'rb'))
    # with open('../Data/batch2_corrected_Qclin.pkl', 'wb') as fp:
    #     pickle.dump(add_bat_Qclin(batch2), fp)
    #
    # batch1 = pickle.load(open('../Data/batch1_corrected.pkl', 'rb'))
    # with open('../Data/batch1_corrected_Qclin.pkl', 'wb') as fp:
    #     pickle.dump(add_bat_Qclin(batch1), fp)


    # Load Data
    batch1_file = './Data/batch1_corrected_Qclin.pkl'
    batch2_file = './Data/batch2_corrected_Qclin.pkl'
    batch3_file = './Data/batch3_corrected_Qclin.pkl'
    ...
    column_names=['dQ_Var', 'dQ_Min', 'dQ_Mean', 'dQ_2V', 'Qd_Intercept_91_100', 'Qd_100', 'Avg_time_first_5', 'IR_2',
     'IR_Diff_2_100', 'cyclelife']

    # for col_idx in range(9):
    #     plt.figure()  # 创建一个新的图形
    #     plt.plot(range(1, 42), trainX[:, col_idx])  # 绘制折线图
    #     plt.plot(range(1,42),trainY_log)
    #     plt.legend([feature[col_idx],'cyclelife'])
    #     plt.title(f'{feature[col_idx]}')  # 设置图形
    #     plt.savefig(f'{feature[col_idx]}.png')# 标题
    #
    # # 显示所有子图
    # plt.show()
