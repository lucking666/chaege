...
from get_imfoemation import extract_delta_Q_variance, extract_fea_for_discharge_model, \
...
def extract_local_fea(batch, index, start_cycle, end_cycle):
    ...
    for ind in index:
        ...
        for cycle in range(start_cycle, end_cycle + 1):
            cycle_temp=[]
            #raw
            cycle_temp.extend(cycle_feat_extract(batch[cell_no]['cycles'][str(cycle)]['Qclin']))
...
def main():
    ...
    for model_choice in range(0,4):
        ...
        # 大小是（41, 100, 21）41个电池，前一百次循环信息，21个特征（7个Qclin，7个Tdlin，7个dQdv）,分别是三个数组的 统计数据min、max、mean、var、median、skew、auc
        ...
        data_path = './Qclin_first_100_cycle_data_' + model_name + '.pt'
